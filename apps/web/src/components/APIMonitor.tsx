...
import { Cpu, CheckCircle2, XCircle, RefreshCw, Server, Shield, Database, Radio } from 'lucide-react';
...
export const APIMonitor: React.FC = () => {
  ...
  const packagesUsed = [
    ...
    { name: 'github.com/Rajangupta9/gopkg/pkg/database', desc: 'MongoDB Connection Pool & Collection Helper' },
  ];
    ...
    <div style={{ padding: '32px 24px', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          ...
          <h1 style={{ fontSize: '28px', fontWeight: 800 }}>Go API & `gopkg` Health Console</h1>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            {health ? (
              ...
              <div style={{ width: '48px', height: '48px', borderRadius: '14px', background: 'rgba(244, 63, 94, 0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '24px' }}>
            <div>
              <div style={{ fontSize: '12px', color: 'var(--text-dim)', fontWeight: 600 }}>API Latency</div>
              <div style={{ fontSize: '20px', fontWeight: 800, color: 'var(--accent-emerald)' }}>{latency !== null ? `${latency} ms` : '—'}</div>
            </div>
            <div>
              <div style={{ fontSize: '12px', color: 'var(--text-dim)', fontWeight: 600 }}>Active Port</div>
              <div style={{ fontSize: '20px', fontWeight: 800, color: 'var(--primary-light)' }}>8080</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel" style={{ padding: '28px' }}>
        <h3 style={{ fontSize: '20px', fontWeight: 700, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Shield size={20} color="var(--primary-light)" /> Integrated `github.com/Rajangupta9/gopkg` Library Modules
        </h3>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '16px' }}>
          {packagesUsed.map((pkg, idx) => (
            <div key={idx} className="glass-card" style={{ padding: '16px' }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '13px', color: 'var(--primary-light)', fontWeight: 700, marginBottom: '6px' }}>
                {pkg.name}
              </div>
